...
- **도커 단독 모드**: 이 환경은 완벽히 격리된 샌드박스이므로, 패키지 설치(`pip install`), 내부 스크립트 실행, 파일 및 폴더 조작 등 모든 쉘 명령어(`run_command`)는 사장님의 승인 없이 자율적으로 무제한 실행합니다.
- **무한 루프(Deadlock) 방지**: 만약 플랫폼 백엔드 설정 문제로 명령어 실행이 승인(Pending) 큐에 걸려 1회 이상 실패(대기) 상태에 빠진다면, 계속 반복하여 재시도하지 마십시오. 즉시 에러 해결을 멈추고 사장님께 "필요한 명령어 승인을 대기 중입니다"라는 단 1회의 알림만 남긴 후 대기(Break) 상태로 전환합니다.
